...
class DynamicTestResultForm(forms.Form):
    test_type = forms.ChoiceField(choices=[(k, k) for k in TEST_PARAMETERS.keys()])

    def __init__(self, *args, **kwargs):
        test_type = kwargs.pop('test_type', None)
        super().__init__(*args, **kwargs)

        if test_type in TEST_PARAMETERS:
            for name, unit in TEST_PARAMETERS[test_type]:
                field_name = f"{name}__{unit}"
                self.fields[field_name] = forms.FloatField(
                    label=f"{name} ({unit})", required=False
                )
