...
def add_test_result(request, patient_id):
    patient = get_object_or_404(Patient, id=patient_id)
    test_type = request.POST.get('test_type') if request.method == 'POST' else None

    # Form khởi tạo với test_type hiện tại để hiển thị đúng các chỉ số
    form = DynamicTestResultForm(request.POST or None, test_type=test_type)
    
    if request.method == 'POST' and form.is_valid():
        selected_type = form.cleaned_data.get('test_type')

        for field_name, value in form.cleaned_data.items():
            if field_name == "test_type" or value in (None, ''):
                continue

            # Tách chỉ số và đơn vị từ field name
            param_name, unit = field_name.split('__')

            # Lưu mỗi chỉ số là một TestResult riêng
            TestResult.objects.create(
                patient=patient,
                test_type=selected_type,
                result_value=value,
                unit=unit,
                price=0  # bạn có thể tính giá nếu cần
            )
