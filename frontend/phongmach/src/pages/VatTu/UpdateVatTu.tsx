...
function UpdateVatTu() {
  const navigate = useNavigate();
  const location = useLocation();
  const { updateVatTu } = useVatTu();
  const [menuOpen, setMenuOpen] = useState(false);
  
  // Get vatTu from location state
  const vatTu = location.state?.vatTu;
  
  // Redirect if no vatTu provided
  useEffect(() => {
    if (!vatTu) {
      navigate('/qlvattu');
    }
  ...
  // Form state
  const [type, setType] = useState(vatTu?.type || typeOptions[0]);
  const [name, setName] = useState(vatTu?.name || nameOptionsByType[typeOptions[0]][0]);
  const [price, setPrice] = useState(vatTu?.price || '');
  const [storage, setStorage] = useState(vatTu?.storage || storageOptions[0]);
  const [dateImport, setDateImport] = useState(vatTu?.dateImport || '');
  const [expiry, setExpiry] = useState(vatTu?.expiry || '');
  const [stock, setStock] = useState(vatTu?.stock || '');
  const [supplier, setSupplier] = useState(vatTu?.supplier || '');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateForm() && vatTu?.id) {
      updateVatTu(vatTu.id, {
        name,
        price,
        type,
        storage,
      ...
      <div style={{ flex: 1, padding: '32px 0 0 0', minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '0 32px', marginTop: 18, marginBottom: 24 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 32, maxWidth: 800, margin: '0 auto', boxShadow: '0 2px 12px #0001' }}>
            <div style={{ fontSize: '2rem', fontWeight: 600, marginBottom: 24 }}>Chỉnh sửa vật tư</div>
